...
public class SuggestionActivity extends AppCompatActivity implements GestureDetector.OnGestureListener {
    ...
    private ConstraintLayout suggestionLayout, onSuccessLayout, onFailureLayout;

    private EditText categoryEditText, quoteEditText, authorEditText;

    private static final String CATEGORY_KEY = "category";
    private static final String QUOTE_TEXT_KEY = "quoteText";
    ...
    private CollectionReference suggestionsCollectionReference = FirebaseFirestore.getInstance().collection("mainCollection")
            .document("suggestions").collection("suggestionsCollection");
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        onSuccessLayout = findViewById(R.id.onSuccessLayoutSuggestion);
        onFailureLayout = findViewById(R.id.onFailureLayoutSuggestion);
    ...
    public void onSubmitButtonClick(View view) {
        ...
        //if quote suggestion text is empty, just exit method with empty return statement
        if(quoteText.isEmpty()) {return; }

        //if quote suggestion text not empty, save suggestion to Maps: "category":"categoryText" - "quoteText":"quoteText" - "author":"authorText" (Map= (key, value) pairs)
        final Map<String, Object> suggestionToSave = new HashMap<>();
        suggestionToSave.put(CATEGORY_KEY, categoryText);
        suggestionToSave.put(QUOTE_TEXT_KEY, quoteText);
        suggestionToSave.put(AUTHOR_KEY, authorText);

        //write the maps into a document in the defined Cloud Firestore database collection (document name = random unique ID)
        suggestionsCollectionReference.add(suggestionToSave).addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
            @Override
    ...
    public void onUpButtonClickSuggestion(View view) {
        finish();
    }
